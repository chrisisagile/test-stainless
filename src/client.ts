...
import { ApplicationCreateParams, ApplicationCreateResponse, ApplicationListParams, ApplicationListResponse, ApplicationRetrieveResponse, ApplicationUpdateParams, ApplicationUpdateResponse, Applications } from './resources/applications';
import { Companies, CompanyListResponse } from './resources/companies';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['CREDEX_UNITY_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['CREDEX_UNITY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CREDEX_UNITY_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Credex Unity API. 
 */
export class CredexUnity {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Credex Unity API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['CREDEX_UNITY_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['CREDEX_UNITY_BASE_URL'] ?? /v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CREDEX_UNITY_BASE_URL'),
    apiKey = readEnv('CREDEX_UNITY_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? CredexUnity.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('CREDEX_UNITY_LOG'), 'process.env[\'CREDEX_UNITY_LOG\']', this) ?? defaultLogLevel;
  ...
  static CredexUnity = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static CredexUnityError = Errors.CredexUnityError;
...
CredexUnity.Applications = Applications;
CredexUnity.Companies = Companies;

export declare namespace CredexUnity {
